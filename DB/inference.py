...
def polygons_from_bitmap(pred, bitmap, dest_width, dest_height, max_candidates=100, box_thresh=0.7):
    ...
    tup = cv2.findContours((bitmap * 255).astype(np.uint8), cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)
    contours = tup[0]
    print(len(contours))
...
if __name__ == '__main__':
    ...
    _, model = dbnet()
    print("###### Model loaded! ######")
    model.load_weights('weights/db_48_2.0216_2.5701.h5', by_name=True, skip_mismatch=True)
    ...
    for image_path in glob.glob(osp.join('datasets/total_text/test_images', '*.jpg')):
        ...
        print("Written: {}".format(image_fname))
